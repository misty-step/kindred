...
try {
  if (process.argv.length !== 2) {
    throw new SetupError(
      "Setup takes no arguments. It uses only this app's initialized development configuration.",
    );
  }
  if (!existsSync(envPath) || !lstatSync(envPath).isFile()) {
    throw new SetupError(
      "First run pnpm dev:backend and select a DEVELOPMENT backend. Keep it running, then run setup again.",
    );
  }
  ...
  if (guestNames.some((name) => Object.hasOwn(process.env, name))) {
    throw new SetupError(
      "Guest configuration is already present in the shell. Unset it before setting up a fresh isolated app; setup will not replace existing keys.",
    );
  }
  ...
  console.log("Start or restart pnpm dev, then open http://localhost:3000.");
} catch (error) {
  console.error(
  ...
  if (saved) {
    console.error(
      "The generated keys are already saved in kindred/.env.local. Do NOT delete this file, rerun setup, or generate replacement keys.",
    );
    console.error(
